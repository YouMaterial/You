...
def update_cmake_version(version: str) -> None:
    ...
    cmake_ver_pattern = r"set\(YOU_VERSION\s+([\d.]+)\)"
    ...
    replaced = re.sub(
        cmake_ver_pattern,
        ver_repl,
        PATHS.ROOT_CMAKE.read_text(),
        count=1,
    )
    PATHS.ROOT_CMAKE.write_text(replaced, "UTF-8")


INIT_FILE = PATHS.YOUCODEGEN_ROOT / "__init__.py"
...
def pprint_release_change_log(
    ...
    current_changes = "".join(
        release_preview.changelog.splitlines()[1:],
    )  # remove release type
